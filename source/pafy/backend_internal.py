...
class InternPafy(BasePafy):
    ...
    def _process_streams(self):
        ...
        audiostreams = sorted(audiostreams, key=lambda x: x.rawbitrate, reverse=True)
        videostreams = sorted(videostreams, key=lambda x: x.dimensions, reverse=True)
    ...
    def url(self):
        ...
        if not self._url:
            if self._parent.age_ver:
                ...
                self._sig = _decodesig(
                    self._sig, self._parent.js_url, self._parent.callback
                )
...
def get_js_sm(watchinfo, callback):
    ...
    if not mainfunc:
        ...
        javascript = fetch_cached(
            js_url, callback, encoding="utf8", dbg_ref="javascript", file_prefix="js-"
        )
