...
class DescriptionDialog(wx.Dialog):
    def __init__(self, parent, content):
        ...
        self.contentBox = wx.TextCtrl(
            panel,
            -1,
            value=self.process(),
            style=wx.TE_PROCESS_ENTER | wx.TE_READONLY | wx.TE_MULTILINE | wx.HSCROLL,
        )
        copyButton = wx.Button(panel, -1, _("نسخ"), name="export")
        txtExport = wx.Button(
            panel, -1, _("التصدير إلى مستند نصي (txt)..."), name="export"
        )
        htmlExport = wx.Button(
            panel, -1, _("التصدير إلى صفحة ويب (html)..."), name="export"
        )
